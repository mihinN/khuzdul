...
from assembler.ir import (
    IRProgram, IRInstructions, IRLabel, IRData, IRDirectives, Instructions,
...
# ****  Parse Directives **** 
    ...
    def _parse_data(self, directive: str, line: int):
        ...
        self._skip_to_newline()
        return IRData(
            label  = None,   # label is attached by symbol
            size   = size,
            values = values,
            line   = line,
        )
...
# **** Memory Operands **** 
    def _parse_memory_operand(self) -> Operand:
        ...
        if self._peek().type == TokensType.REGISTER:
            ...
            disp = self._to_int(self._peek_advance().value)  
        # remaining term 
...
# **** helper functions ****
    def _to_int(self, value: str) -> str: 
    ...
    def _consume(self, expected: TokensType) -> TokensType:
        tok = self._peek()
    ...
    def _at_end(self) -> bool:
        return self.tokens[self.position].type == TokensType.EOF  